...
function CauseCard({ num, title, solution, children }) {
  return (
...
export default function BenchPressPlateauCauses() {
  return (
    ...
    >
      ...
      <CauseCard num="1" title="毎回同じ強度で追い込んでいる" solution="週によって強度を変える。重い週の翌週は意図的に軽くする。">
        最もよくある原因です。毎回「限界の重量で限界まで」やり続けると、神経系が疲弊して重量が落ちていきます。
      </CauseCard>
      <CauseCard num="2" title="セットインターバルが短すぎる" solution="BIG3のセット間は最低3分、できれば5分確保する。">
        インターバルを短くすることは、次のセットのパフォーマンスを大きく落とします。研究では、セット間3〜5分の休憩が筋力・筋肥大の両方に有効であることが示されています。
      </CauseCard>
      <CauseCard num="3" title="補助種目が足りない" solution="ナローグリップベンチ・ディップス・フェイスプルなどを週1〜2回追加する。">
        ベンチプレスの停滞には、三頭筋・前鋸筋・ローテーターカフの弱さが関係していることが多いです。胸だけを鍛えても、補助する筋群が弱ければ頭打ちになります。
      </CauseCard>
      <CauseCard num="4" title="タンパク質・カロリーが不足している" solution="体重×1.6gのタンパク質を毎日確保する。カロリーは維持〜微増が基本。">
        栄養が足りない状態では、いくらトレーニングしても筋肉が成長しません。特に増量期でないのに体重が下がっているときは要注意です。
      </CauseCard>
      <CauseCard num="5" title="疲労が蓄積している" solution="1週間のデロード（強度を60〜70%に落とす週）を挟む。">
